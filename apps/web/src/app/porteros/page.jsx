...
export default function PaginaPorteros() {
  ...
  const limpiarResultadoAutomaticamente = useCallback(() => {
    if (timeoutRef.current) {
      clearTimeout(timeoutRef.current)
    }
  ...
  const procesar = useCallback(
    async (token) => {
      ...
      try {
        ...
        limpiarResultadoAutomaticamente()
      } catch (error) {
        ...
        const entradaError = {
          ...
          valido: false,
          motivo: 'Error al verificar la entrada',
        }

        if (!mountedRef.current) return

        setResultado(entradaError)
        setHistorial((prev) => [entradaError, ...prev])
    ...
    [isLoading, limpiarResultadoAutomaticamente]
  )
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-4">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 sm:p-6">
            {modo === 'camara' ? (
              <CamaraScanner onScan={procesar} />
            ) : (
              <div className="flex flex-col gap-4">
                <p className="text-zinc-500 text-sm text-center">
                  Pega o escribe el token del QR
                </p>
        ...
        <div>
          ...
          {historial.length === 0 && (
            <p className="text-zinc-600 text-sm">
              Aún no hay validaciones en esta sesión.
            </p>
          )}
